...
class Navbar2 extends Component {
  ...
  renderLinks = () => {
    ...
    if (!user) return null; // Checks if the user object exists before rendering links
    ...
    if (user.isLogin) {
      if (user.isRestaurant) {
        links = [
          { path: "/add-menu-items", label: "Add Foods" },
          { path: "/my-foods", label: "My Foods" },
          { path: "/order-requests", label: "Order Requests" },
        ];
      } else {
        links = [
          { path: "/restaurants", label: "Restaurants" },
      ...
      <>
        {user.isLogin && (
          <Nav.Item className="nav-item">
            <span className="navbar-text text-uppercase mr-2">
        ...
        {links.map((link) => (
          <Nav.Item key={link.label} className="nav-item">
            {link.isButton ? (
              <button
                type="button"
                className="btn btn-warning btn-sm text-uppercase mr-2 mr-1 px-3"
    ...
    return (
      ...
      >
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="ml-auto">{this.renderLinks()}</Nav>
        </Navbar.Collapse>
      </Navbar>
    );
  }
...
const mapStateToProps = (state) => ({
  user: state.user || {}, // Ensure user object defaults to an empty object if undefined
});
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Navbar2)
);
