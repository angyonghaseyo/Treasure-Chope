      ...
      firebase
        ...
        .then((docSnapshot) => {
          if (docSnapshot.exists) {
            this.setState({
              userFavorites: docSnapshot.data().userFavorites || [],
            });
          }
        })
        .catch((error) => {
  ...
  handleSearchBar(event) {
    ...
    if (restaurantList) {
      const result = restaurantList.filter((val) =>
        val.userName.toLowerCase().includes(searchText.toLowerCase())
      );
      this.setState({
  ...
  changeSortingMethod = (method) => {
    this.setState((prevState) => {
      if (method === "favorites") {
        return {
          sortingMethod:
            prevState.sortingMethod === "favorites" ? "all" : "favorites",
        };
      }
      // Toggle between ascending and descending if alphabetical sorting is already selected
      if (
        method === "alphabetical" &&
        prevState.sortingMethod.includes("alphabetical")
      ) {
        return {
          sortingMethod:
            prevState.sortingMethod === "alphabetical_asc"
              ? "alphabetical_desc"
              : "alphabetical_asc",
        };
      }
  ...
  _renderRestaurantList() {
    ...
    if (restaurantList) {
      ...
      if (sortingMethod === "favorites") {
        sortedRestaurantArray = restaurantList.filter((restaurant) =>
          userFavorites.includes(restaurant.id)
        );
      } else if (sortingMethod.includes("alphabetical")) {
        sortedRestaurantArray.sort((a, b) =>
          sortingMethod === "alphabetical_asc"
          ...
          <div className="container bg-white p-3 px-0 mb-4" key={restaurant.id}>
            ...
            >
              ...
              <div className="col-lg-3 col-md-3 col-sm-12 d-flex justify-content-center flex-column px-0">
                ...
                >
                  <span
                    onClick={() => this.toggleFavorite(restaurant.id)}
                    style={{
                      cursor: "pointer",
                      textAlign: "center",
                      borderRadius: "13px",
              ...
              <div className="col-lg-3 col-md-3 col-sm-12">
                ...
                <div className="container bg-white py-3 sort-by">
                  ...
                  <ul>
                    ...
                    <li onClick={() => this.changeSortingMethod("favorites")}>
                      <FontAwesomeIcon icon={faHeart} className="mr-3" />
                      Favorites{" "}
                      {this.state.sortingMethod === "favorites"
                        ? "(Filtered)"
                        : ""}
                    </li>
                  </ul>
                ...
                {/* <div>
                  {this.state.renderSearchRestaurants
                    ? this._renderSearchRestaurants()
